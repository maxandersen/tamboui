...
* Building from source: JDK 22+ (JDK 23+ required to generate Javadoc from `///` Markdown comments)

Core library artifacts (such as `tamboui-core`, `tamboui-widgets`, and `tamboui-tui`)
are compiled with `--release 8` and published as multi-release JARs with
