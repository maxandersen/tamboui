...
== Markup and RichIO (CLI Output)

TamboUI includes a small Rich / BBCode-inspired markup parser for producing styled `Text` without requiring widgets or a full-screen TUI:
...
For direct terminal output, use `RichIO`, which can print markup strings, `Text`, and pretty formatted `Throwable`s:

[source,java]
----
import dev.tamboui.console.RichIO;

var console = RichIO.system();
console.print("[link=https://example.com]Hello[/link]");
