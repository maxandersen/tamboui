...
public final class FigletDemo {
    ...
    private static final List<BundledFigletFont> FONTS = List.of(
        BundledFigletFont.BIG,
        BundledFigletFont.SLANT,
        BundledFigletFont.STANDARD,
        BundledFigletFont.SMALL,
        BundledFigletFont.MINI
    );
    ...
    private void renderFontList(Frame frame, Rect area) {
        List<ListItem> items = FONTS.stream()
            .map(f -> ListItem.from(f.id() + " (" + FigletFont.bundled(f).height() + "h)"))
            .toList();
        ...
        ListWidget list = ListWidget.builder()
            ...
            .block(Block.builder()
                ...
                .titleBottom(Title.from("↑↓ / j k").right())
                .build())
            .highlightSymbol("▶ ")
    ...
    private void renderMainFiglet(Frame frame, Rect area) {
        BundledFigletFont selected = selectedFont();
        String text = inputState.text().isBlank() ? "TamboUI" : inputState.text();

        Color color = PALETTE[Math.floorMod(selected.ordinal(), PALETTE.length)];
    ...
    private BundledFigletFont selectedFont() {
        Integer sel = fontListState.selected();
        if (sel == null) {
