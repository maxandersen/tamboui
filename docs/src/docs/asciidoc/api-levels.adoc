...
Elements can emit typed events through the element event bus:

[source,java]
----
try (var runner = ToolkitRunner.create()) {
    runner.elementEventBus().subscribe(TextInputSubmittedEvent.class, event -> {
        if ("search".equals(event.sourceId())) {
    ...
    runner.run(() -> textInput(inputState).id("search"));
}
----

Built-in elements emit typed events such as `TextInputChangedEvent`,
`TextInputSubmittedEvent`, and `TextAreaChangedEvent`.
